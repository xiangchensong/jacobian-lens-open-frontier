...
class ActivationRecorder:
    ...
    Args:
        ...
        extra_modules: Modules outside ``blocks``, keyed by the *virtual* index
            they are addressed with. The convention is ``n_layers`` for a
            module that runs immediately after the last block — DeepSeek-V4's
            ``hc_head``, whose output is the Jacobian target. Indices given
            here take precedence over ``blocks``, so ``at`` may mix real and
            virtual indices freely.
    """
    ...
    def __init__(
        ...
        *,
        ...
        extra_modules: Mapping[int, nn.Module] | None = None,
    ) -> None:
        self._blocks = blocks
        self._extra_modules = dict(extra_modules or {})
    ...
    def _module_at(self, index: int) -> nn.Module:
        if index in self._extra_modules:
            return self._extra_modules[index]
        ...
        try:
            for index in self._indices:
                self._handles.append(
                    self._module_at(index).register_forward_hook(self._make_hook(index))
                )
        except Exception:
