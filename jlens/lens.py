...
class JacobianLens:
    ...
    Attributes:
        jacobians: ``{layer_index: Tensor[d_model, d_source]}``. Each ``J_l``
        ...
        d_model: Width of the transported (target) vector, i.e. what
            :meth:`~jlens.protocol.LensModel.unembed` consumes. Kept under this
            name because it is the serialised field, and for a single-stream
            model it *is* the residual-stream width.
        d_source: Flattened width of the residual the lens reads from. Equal to
            :attr:`d_model` unless the model carries several parallel residual
            streams per position (see :class:`jlens.protocol.LensModel`).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.d_source = d_model if d_source is None else d_source

    def __repr__(self) -> str:
        shape = (
            f"d_model={self.d_model}"
            if self.d_source == self.d_model
            else f"d_model={self.d_model}, d_source={self.d_source}"
        )
        return (
            f"JacobianLens({shape}, n_prompts={self.n_prompts}, "
    ...
    def load(cls, path: str) -> JacobianLens:
        ...
        return cls(
            ...
            # Absent in lenses saved before rectangular J_l; those are square.
            d_source=checkpoint.get("d_source"),
        )
    ...
    def merge(cls, lenses: Sequence[JacobianLens]) -> JacobianLens:
        ...
        Args:
            lenses: Lenses to merge. Must agree on ``source_layers``,
                ``d_model`` and ``d_source``.
        ...
        for other in lenses[1:]:
            if (
                ...
                or other.d_source != first.d_source
            ):
                raise ValueError("lenses disagree on source_layers / d_model / d_source")
    ...
    def transport(self, residual: torch.Tensor, layer: int) -> torch.Tensor:
        ...
        Args:
            residual: Tensor of shape ``[..., d_source]``, or the uncollapsed
                ``[..., n_streams, d]`` a multi-stream block emits — the
                trailing axes are flattened row-major, matching the flatten the
                fit applied to the gradients.
            layer: Source layer index (must be in :attr:`source_layers`).

        Raises:
            ValueError: If the residual cannot be read as ``d_source`` wide.
        """
        J_bar = self.jacobians[layer].to(residual.device)
        if residual.shape[-1] != self.d_source and residual.dim() >= 2:
            residual = residual.flatten(start_dim=-2)
        if residual.shape[-1] != self.d_source:
            raise ValueError(
                f"residual of shape {tuple(residual.shape)} is not d_source="
                f"{self.d_source} wide; this lens was fitted on a different model"
            )
    ...
    def apply(
        ...
        *,
        ...
        def select(layer: int) -> torch.Tensor:
            """Residuals at the requested positions: ``[n_positions, ...]``.

            The trailing axes are whatever the block emits — ``[d_model]`` for a
            single-stream model, ``[n_streams, d]`` for a multi-stream one.
            :meth:`transport` and ``model.unembed`` both accept either.
            """
            full = activations[layer][0]  # [seq_len, ...]
