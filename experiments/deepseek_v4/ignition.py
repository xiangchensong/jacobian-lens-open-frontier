"""Experiment 6.3 -- ignition: how sharply the band commits to one reading of an
ambiguous token, plus 1.2 (introspection).
...
def exp63():
    ...
    json.dump({str(k): v for k, v in out.items()},
              open(f"{SCRATCH}/exp63_ignition.json", "w"), indent=1)
