"""Experiment 6.2 on the preview checkpoint, read with its own band lens -- the
preview side of the band comparison. Path-only derivation of
deepseek_v4/workspace_band.py; note the band lens covers L19-39 only, so an
onset earlier than L19 is undetectable here.
...
FULL_LENS = "/data3/fan-test/jlens_out/v4preview_band_n100_s128.pt"
BAND_LENS = "/data3/fan-test/jlens_out/v4preview_band_n100_s128.pt"
DEV = "cuda:0"
...
def onset_offset(lens, J):
    ...
    with open(f"{SCRATCH}/exp62_stats_preview.json", "w") as f:
        json.dump(rows, f, indent=1)
    log(f"wrote {SCRATCH}/exp62_stats_preview.json")
...
six_set_eval(band, Jb, "v4preview_band_n100_s128 (L19-39)")
