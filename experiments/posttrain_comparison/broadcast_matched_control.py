...
summoning) on DeepSeek-V4-Flash-0731 with the n=100 lens -- the matched control
for broadcast_preview.py, from which it differs only in the MODEL and LENS
constants. Running both checkpoints through one identical code path is what
exposed the silent clamp fallback: the preview's 0/192 became legible as a
script bug only when 0731 -- whose published transfer run scores 0.208 --
returned 0/192 too. Re-derived from the fixed deepseek_v4/broadcast.py.
