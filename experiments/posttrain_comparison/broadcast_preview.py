...
summoning) on the DeepSeek-V4-Flash *preview* checkpoint with its own band lens
-- the recipe-axis counterpart of the 0731 run behind the reported 0.208.
Re-derived from the fixed deepseek_v4/broadcast.py, changing only the MODEL and
LENS constants: the first preview attempt used the pre-fix script and was
invalidated by the silent clamp fallback the hard error below now prevents.
